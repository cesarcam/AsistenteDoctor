...
//Adapter by Shuirna, incorporate it to QML canvas
//NOTE: This modification are inspired by QChart.js(Author: Julien Wintz)
//I'm using the newest version of Chart.js(This version is a huge change compared to the previous version
//SO it has all the new features such as Tooltips and future extension
//Some import Modification:
// 1. replace aninmation with QML's property animation
// 2. bind mouse event with QML's mouseArea
// 3. replace window object with a global variable(root) for QML

//Imitiate window object in the browser
var root;

function newChartInstance(context, data, options, chartType) {
    var ChartInstance = root;
    if(chartType === "Bar") {
        ...
        return new ChartInstance(context).Pie(data, options);
    } else if(chartType === "PolarArea") {
        return new ChartInstance(context).PolarArea(data, options);
    } else if(chartType === "Radar") {
        return new ChartInstance(context).Radar(data, options);
...
//			this.render();
        ...
        bindMouseEvent: function(event) {
             if (this.options.showTooltips){
                      var activeBars = (event.type !== 'mouseout') ? this.getBarsAtEvent(event) : [];
                      this.eachBars(function(bar){
                          bar.restore(['fillColor', 'strokeColor']);
                      });
                      helpers.each(activeBars, function(activeBar){
...
//			this.render();
        ...
        bindMouseEvent : function(event) {
            if (this.options.showTooltips){
                var activeSegments = (event.type !== 'mouseout') ? this.getSegmentsAtEvent(event) : [];
...
//			this.render();
        ...
        bindMouseEvent: function(event) {
            //Set up tooltip events on the chart
            if (this.options.showTooltips){
                var activePoints = (event.type !== 'mouseout') ? this.getPointsAtEvent(event) : [];
                this.eachPoints(function(point){
                    point.restore(['fillColor', 'strokeColor']);
                });
                helpers.each(activePoints, function(activePoint){
...
//			this.render();
        ...
        bindMouseEvent : function(event) {
            if (this.options.showTooltips){
                var activePointsCollection = (event.type !== 'mouseout') ? this.getPointsAtEvent(event) : [];
